...
-- Crosswords
grant select on table public.users            to report_ro;
...
grant select on table public.ghosts           to report_ro;
grant select on table public.named_ghosts     to report_ro;
-- Make Me Red
grant select on table public.mmr_results      to report_ro;
grant select on table public.mmr_profiles     to report_ro;

-- Nothing this role did not get explicitly, now or in future.
...
-- Verify: every table should list SELECT for report_ro and a "report_ro: read" policy.
